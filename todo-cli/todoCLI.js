...
function todoView() {
  ...
  for (let index = 0; index < todoList.length; index++) {
    ...
    console.log(
      `${index} [${todoEntry.completed ? "✓" : " "}] ${todoEntry.title}`
    );
  }
  console.log("");
}
...
function isValidTodoIndex(ansNum) {
  if (Number.isInteger(parseInt(ansNum, 10))) {
    const index = parseInt(ansNum, 10);
...
function todoMenu() {
  ...
  rl.question("> ", (answer) => {
    if (answer == "v") {
      ...
      answer.toString().charAt(0) == "c" &&
      isValidTodoIndex(answer.toString().slice(1))
    ) {
      todoComplete(parseInt(answer.toString().slice(1), 10));
      todoMenu();
    } else if (
      answer.toString().charAt(0) == "d" &&
      isValidTodoIndex(answer.toString().slice(1))
    ) {
